...
function booksHandler() {
  this.updateOneUser = function (req, res) {
    ...
    User.findOne({ 'local.username': req.user.local.username }).exec(function (err, result) {
      if (err) throw err;
      if (JSON.stringify(result) !== userStringifiedOriginal) {
        res.end(refreshMessage);
      } else {
        User.findOneAndUpdate({ 'local.username': req.user.local.username }, { 'local.books': user.local.books, 'local.isbns': user.local.isbns }, { new: true }).exec(function (err, doc) {
          if (err) throw err;
          res.json(doc);
        });
      }
  ...
  this.getMyBooks = function (req, res) {
    User.findOne({ 'local.username': req.user.local.username }).exec(function (err, user) {
      if (err) throw err;
      res.json([user]);
    });
  };
  ...
  this.filteredSearch = function (req, res) {
    ...
    User.find({ "$or": searchFilterArray }).exec(function (err, users) {
      if (err) throw err;
      res.json(users);
    });
  };
  this.updateTwoUsers = function (req, res) {
    ...
    User.find({ "$or": [{ 'local.username': userA.local.username }, { 'local.username': userB.local.username }] }).exec(function (err, users) {
      if (err) throw err;
      if (JSON.stringify(users[0]) !== userAStringifiedOriginal && JSON.stringify(users[0]) !== userBStringifiedOriginal || JSON.stringify(users[1]) !== userBStringifiedOriginal && JSON.stringify(users[1]) !== userAStringifiedOriginal) {
        ...
        User.findOneAndUpdate({ 'local.username': userA.local.username }, { 'local': userA.local }, { new: true }).exec(function (err, docA) {
          if (err) throw err;
          User.findOneAndUpdate({ 'local.username': userB.local.username }, { 'local': userB.local }, { new: true }).exec(function (err, docB) {
            if (err) throw err;
            res.json([docA, docB]);
          });
        });
...
//# sourceMappingURL=booksHandler.server.js.map
